  ...
  sql = <<-EOQ
    ...
    where
      case 
        ...
        and (author_name in (select unnest (string_to_array($3, ',')::text[])) or author_name = '<No author>'))
        ...
        then feed_link = $1 and (author_name in (select unnest (string_to_array($3, ',')::text[])) or author_name = '<No author>')
